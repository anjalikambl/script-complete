...
plot(weight,chol,main="w vs chol",xlab="weight",ylab="chol",col="blue",las=1,col.main=2,font.main=3,col.axis=4,col.lab=3)
par(bg="yellow")

points(chol[gender=="yes"]~weight[gender=="yes"],pch=16)
abline(lm(chol[gender=="no"]~weight[gender=="no"]),col=3)
...
library(manipulate)
manipulate(plot(cars,xlim=c(x.min,x.max),type=plot_type,axes=axes,ann=label),x.min=slider(0,15),x.max=slider(15,30,initial=25),plot_type=picker("p","l","b"),axes=checkbox(TRUE,"Axes"),label=checkbox(TRUE,"Labels"))
...
prop.test(550,1000,p=0.55,conf.level=0.99)#ztest for proportionality
...
gvlma(m3)#giving linear model assumptions
